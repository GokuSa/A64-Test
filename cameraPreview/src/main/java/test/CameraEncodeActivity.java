 ...
 */
public class CameraEncodeActivity extends Activity implements SurfaceTexture.OnFrameAvailableListener {
    public static final String TAG = "CameraEncodeActivity";
    ...
    //摄像头编码开关自动切换
    private boolean autoEncodeSwitch = false;
    //VLC直播流开关自动切换
    private boolean autoPlaySwitch = false;

    private boolean isPlaying = false;
    private EditText mEditTextUrl;
    private Button mBtnTogglePlay;
    private static final int SWITCH_DURATION = 30 * 1000;
...
//        startSwitch=false;
    ...
    static class CameraHandler extends Handler {
        ...
        private WeakReference<CameraEncodeActivity> mWeakActivity;

        public CameraHandler(CameraEncodeActivity activity) {
            mWeakActivity = new WeakReference<CameraEncodeActivity>(activity);
        }
        ...
        public void handleMessage(Message inputMessage) {
            ...
            CameraEncodeActivity activity = mWeakActivity.get();
            if (activity == null) {
    ...
    private void handleAutoEncodeSwitch() {
        //自动切换摄像头编码
        clickToggleRecording(null);
        if (autoEncodeSwitch) {
            int duration = mRecordingEnabled ? 60 * 1000 : 2 * 1000;
            mCameraHandler.sendEmptyMessageDelayed(CameraHandler.MSG_AUTO_ENCODE_SWITCH, duration);
        }
    }

    private void handleAutoPlaySwitch() {
        clickTogglePlay(null);
        if (autoPlaySwitch) {
            //播10秒 停2秒
            int duration = isPlaying ? 10 * 1000 : 2000;
            mCameraHandler.sendEmptyMessageDelayed(CameraHandler.MSG_AUTO_PLAY_SWITCH, duration);
        }
    }
    ...
    class CameraSurfaceRenderer implements GLSurfaceView.Renderer {
        ...
        private CameraEncodeActivity.CameraHandler mCameraHandler;
         ...
         */
        public CameraSurfaceRenderer(CameraEncodeActivity.CameraHandler cameraHandler,
        ...
        public void onSurfaceCreated(GL10 unused, EGLConfig config) {
            ...
            mCameraHandler.sendMessage(mCameraHandler.obtainMessage(
                    CameraEncodeActivity.CameraHandler.MSG_SET_SURFACE_TEXTURE, mSurfaceTexture));
